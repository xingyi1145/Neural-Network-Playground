...
from backend.api.templates import (
    TEMPLATES,
    filter_templates_by_dataset,
    get_template_by_id,
    list_all_templates,
)
...
def test_all_datasets_have_simple_and_deep_templates() -> None:
    ...
    for dataset_id, templates in by_ds.items():
        ...
        assert any(
            "simple" in tid for tid in ids
        ), f"{dataset_id} missing simple template"
...
def test_template_shapes_match_datasets() -> None:
    ...
    for template_id, template in TEMPLATES.items():
        ...
        expected_output_dim = (
            dataset.num_classes if dataset.task_type == "classification" else 1
        )
        ...
        assert (
            layers[-1]["type"] == "output"
        ), f"{template_id}: Last layer must be output"
