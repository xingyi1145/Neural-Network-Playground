...
class MockDataset:
    def __init__(self, task_type="classification", features=10, samples=100, classes=2):
        ...
        self.hyperparameters = Hyperparameters(epochs=2, learning_rate=0.01, batch_size=10)
...
def test_train_small_classification(mock_get_dataset, simple_model_config):
    mock_get_dataset.return_value = MockDataset(task_type="classification", features=10, samples=50, classes=2)
    engine = TrainingEngine(dataset_id="mock_classification", model_config=simple_model_config)
...
def test_train_small_regression(mock_get_dataset):
    mock_get_dataset.return_value = MockDataset(task_type="regression", features=10, samples=50)
    model_config = {
        "layers": [
            {"type": "input", "neurons": 10, "activation": None},
            {"type": "hidden", "neurons": 20, "activation": "relu"},
