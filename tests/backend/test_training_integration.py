...
class TestTrainingHappyPath:
    ...
    def test_create_train_poll_verify_iris(self, test_client, dataset_model_configs):
        ...
        assert create_response.status_code == 201, f"Failed to create model: {create_response.text}"
        ...
        assert train_response.status_code == 202, f"Failed to start training: {train_response.text}"
        ...
        start_time = time.time()
        final_status = wait_for_training_completion(test_client, model_id, max_wait_time=60)
    ...
    def test_training_all_datasets(self, test_client, dataset_model_configs, dataset_id):
        """
        ...
        # Create model
        create_response = test_client.post("/api/models", json=config)
        assert create_response.status_code == 201, f"Failed to create model for {dataset_id}: {create_response.text}"
        model_id = create_response.json()["id"]
        
        # Start training
        train_response = test_client.post(f"/api/models/{model_id}/train")
        assert train_response.status_code == 202, f"Failed to start training for {dataset_id}: {train_response.text}"
        
        # Poll until completion
        start_time = time.time()
        final_status = wait_for_training_completion(test_client, model_id, max_wait_time=300)
        training_time = time.time() - start_time
        
        # Verify completion
        assert final_status["status"] == "completed", f"Training failed for {dataset_id}: {final_status}"
        assert "metrics" in final_status
...
class TestErrorScenarios:
    ...
    def test_nonexistent_model_404(self, test_client):
        ...
        assert response.status_code == 404, f"Expected 404 for GET, got {response.status_code}"
        
        # Get training status
        response = test_client.get(f"/api/models/{fake_model_id}/status")
        assert response.status_code == 404, f"Expected 404 for status, got {response.status_code}"
        
        # Start training
        response = test_client.post(f"/api/models/{fake_model_id}/train")
        assert response.status_code == 404, f"Expected 404 for train, got {response.status_code}"
...
class TestConcurrentTraining:
    ...
    def test_concurrent_model_creation(self, test_client, dataset_model_configs):
        ...
        assert len(model_ids) == num_models, f"Expected {num_models} models, got {len(model_ids)}"
    ...
    def test_multiple_models_training_concurrently(self, test_client, dataset_model_configs):
        ...
        def train_model(dataset_id: str, model_id: str):
            ...
            assert final_status["status"] == "completed", f"Training failed for {dataset_id}"
...
class TestTrainingMetrics:
    ...
    def test_training_metrics_present(self, test_client, dataset_model_configs):
        ...
        if config["dataset_id"] in ["iris", "wine_quality", "mnist", "synthetic"]:
            assert "accuracy" in metrics or "train_accuracy" in metrics or "val_accuracy" in metrics
